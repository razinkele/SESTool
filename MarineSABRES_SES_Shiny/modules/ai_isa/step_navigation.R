...
setup_step_navigation <- function(rv, session, i18n, QUESTION_FLOW) {
  ...
  move_to_next_step <- function() {
    # Log entry for debugging step skipping issues
    debug_log(sprintf("[AI ISA NAV] move_to_next_step called: current_step=%d, caller=%s",
                      rv$current_step, paste(sys.call(-1), collapse=" ")), "AI ISA")

    # Check if we can move forward before incrementing
    if (rv$current_step < length(QUESTION_FLOW)) {
      rv$current_step <- rv$current_step + 1
      debug_log(sprintf("[AI ISA NAV] Step advanced to %d", rv$current_step), "AI ISA")
    }
