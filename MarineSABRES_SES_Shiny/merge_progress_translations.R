#!/usr/bin/env Rscript
# Merge progress indicator translations into main translation.json

library(jsonlite)

cat("Merging progress indicator translations...\n\n")

# Load files
main_trans <- fromJSON('translations/translation.json', simplifyDataFrame = FALSE)
progress_trans <- fromJSON('progress_translations.json', simplifyDataFrame = FALSE)
...
cat("Progress translations to add:", length(progress_trans$translation), "\n\n")

# Add new translations
new_count <- 0
for (entry in progress_trans$translation) {
