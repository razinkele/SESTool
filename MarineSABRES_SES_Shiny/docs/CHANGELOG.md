...
### Added
- **Documentation** - PDF versions of user manuals
  - Converted HTML manuals to PDF using Chrome headless rendering
  - English manual: 737 KB PDF (75 pages)
  - French manual: 617 KB PDF (75 pages)
  - Added PDF download links to About modal alongside HTML versions
  - Users can now access manuals in both HTML (interactive) and PDF (printable) formats

### Fixed
- **Analysis Tools Module** - Removed duplicate broken `box()` definition in `analysis_tools_module.R` (line 1916-1925)
  - Fixed syntax error with incomplete `valueBoxOutput` string
  - Eliminated duplicate UI code block causing parsing errors
- **Report Generation** - Fixed loop detection in dynamically generated reports
  - Added `get_loop_data()` helper function for backward-compatible data structure access
  - Updated 3 report sections: executive summary, full report overview, loop analysis
  - Reports now correctly detect loops stored in both `data$data$analysis$loops$loop_info` (new) and `data$data$cld$loops` (legacy)
- **Documentation Access** - Fixed inactive documentation links for English and French user manuals
  - Added `addResourcePath("docs", "docs")` to `app.R` to make docs directory accessible
  - HTML manual files now properly served from `/docs/` URL path
- **Resource Path Configuration** - Enhanced Shiny resource serving for non-www directories
  - Ensures documentation and other assets outside www/ folder are accessible to users

### Technical
- Improved data structure migration path for legacy project files
- Enhanced error resilience in report generation pipeline
