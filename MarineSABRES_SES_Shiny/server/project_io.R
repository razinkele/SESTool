    ...
    tryCatch({
      ...
      tryCatch({
        ...
        if (is.null(loaded_min)) {
          # Pre-WP5 project loading in a WP5-capable build. Show one-time
          # forward-load toast and stamp min_app_version so re-load doesn't
          # repeat the toast. "1.12.0" marks the schema version that introduced
          # the field (not the running app version).
          showNotification(
