        ...
        fluidRow(
          box(
            ...
            conditionalPanel(
              ...
              h4("Data Summary"),
              fluidRow(
                column(3, valueBoxOutput("numConnections", width = NULL)),
                column(3, valueBoxOutput("numNodes", width = NULL)),
                column(3, valueBoxOutput("strengthTypes", width = NULL)),
                column(3, valueBoxOutput("boolNetStatus", width = NULL))
              ),
              ...
              # Show Boolean network status after processing
              conditionalPanel(
                condition = "output.dataProcessed",
                br(),
                div(class = "alert alert-success",
                    h4("✓ Data Processing Complete"),
                    p("• SES matrix created successfully"),
                    p("• Boolean network file generated"),
                    p("• Network is ready for analysis")
                )
              )
            )
          )
      ...
      tabItem(
        ...
        fluidRow(
          ...
          box(
            ...
            # Add Boolean Network Information
            conditionalPanel(
              condition = "output.dataProcessed",
              br(),
              h4("Boolean Network Info"),
              tableOutput("booleanNetworkInfo")
            )
          )
        )
      ),
      ...
      tabItem(
        ...
        fluidRow(
          box(
            ...
            p("Note: Boolean network file was automatically created during data processing."),
        ...
        fluidRow(
          box(
            ...
            downloadButton("exportNetwork", "Export Network (GraphML)", 
                         class = "btn-info"),
            br(), br(),
            downloadButton("exportBooleanNetwork", "Export Boolean Network", 
                         class = "btn-warning"),
            br(), br(),
...
server <- function(input, output, session) {
  ...
  values <- reactiveValues(
    original_data = NULL,
    ses_matrix = NULL,
    boolean_network = NULL,
    boolean_file_path = NULL,
  ...
  output$dataProcessed <- reactive({
    return(!is.null(values$ses_matrix) && !is.null(values$boolean_network))
  })
  ...
  # Boolean Network Status Box
  output$boolNetStatus <- renderValueBox({
    if (!is.null(values$boolean_network)) {
      valueBox(
        value = "✓",
        subtitle = "BoolNet Ready",
  ...
  # Process data - MODIFIED TO INCLUDE BOOLEAN NETWORK CREATION
  observeEvent(input$processData, {
    ...
    withProgress(message = 'Processing data...', value = 0, {
      ...
      tryCatch({
        ...
        incProgress(0.2, detail = "Creating network matrix...")
        
        # Create SES matrix
        values$ses_matrix <- data.load(processed_data, folder = tempdir(), 
                                     graph = FALSE)
        
        incProgress(0.4, detail = "Generating Boolean network file...")
        
        # Create Boolean network file automatically
        boolean_filename <- "network_boolean"
        boolean_df <- boolean.file.creation(values$ses_matrix, tempdir(), boolean_filename)
        values$boolean_file_path <- file.path(tempdir(), paste0(boolean_filename, ".csv"))
        
        incProgress(0.6, detail = "Loading Boolean network...")
        
        # Load the Boolean network using BoolNet
        values$boolean_network <- loadNetwork(values$boolean_file_path)
        ...
        log_message("Boolean network file created and loaded")
        showNotification("Data processed successfully! Boolean network ready for analysis.", type = "message")
  ...
  # Boolean Network Information Table
  output$booleanNetworkInfo <- renderTable({
    req(values$boolean_network)
    
    # Extract basic Boolean network information
    num_genes <- length(values$boolean_network$genes)
    num_interactions <- sum(sapply(values$boolean_network$interactions, length))
    
    info_df <- data.frame(
      Property = c("Number of Genes", "Total Interactions", "Network Type", "File Location"),
      Value = c(
        num_genes,
        num_interactions,
        "Boolean Network",
        basename(values$boolean_file_path)
      )
    )
  ...
  # Qualitative Analysis - MODIFIED TO USE PRE-LOADED BOOLEAN NETWORK
  observeEvent(input$runQualitative, {
    req(values$ses_matrix, values$boolean_network)
    ...
    withProgress(message = 'Running qualitative analysis...', value = 0, {
      ...
      tryCatch({
        ...
        laplacian <- SES.laplacian(values$ses_matrix, from = "rows")
        
        incProgress(0.5, detail = "Analyzing Boolean network...")
        
        # Run Boolean analysis using the pre-loaded network
        boolean_results <- boolean.analyses(values$boolean_network, tempdir(), "boolean_analysis")
        
        incProgress(0.8, detail = "Finalizing results...")
        
        # Store results
        values$qualitative_results <- list(
          laplacian = laplacian,
          boolean.network = values$boolean_network,
          boolean.results = boolean_results
        )
  ...
  output$analysisSummary <- renderTable({
    analyses <- c("Data Processing", "Boolean Network", "Qualitative Analysis", "Quantitative Analysis", "Intervention Analysis")
    status <- c(
      ifelse(!is.null(values$ses_matrix), "✓ Complete", "✗ Pending"),
      ifelse(!is.null(values$boolean_network), "✓ Complete", "✗ Pending"),
  ...
  output$exportResults <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (!is.null(values$boolean_file_path) && file.exists(values$boolean_file_path)) {
        boolean_file <- file.path(temp_dir, "boolean_network.csv")
        file.copy(values$boolean_file_path, boolean_file)
        files_to_zip <- c(files_to_zip, boolean_file)
      }
  ...
  # New export function for Boolean network
  output$exportBooleanNetwork <- downloadHandler(
    filename = function() {
      paste("boolean_network_", Sys.Date(), ".csv", sep = "")
    },
    content = function(file) {
      req(values$boolean_file_path)
      if (file.exists(values$boolean_file_path)) {
        file.copy(values$boolean_file_path, file)
      }
    }
  )
  ...
  output$exportReport <- downloadHandler(
    ...
    content = function(file) {
      boolean_status <- if (!is.null(values$boolean_network)) "Generated and loaded" else "Not available"
      ...
      html_content <- paste0(
        ...
        "<h2>Boolean Network Status</h2>",
        "<p>Status: ", boolean_status, "</p>",
        if (!is.null(values$boolean_file_path)) paste0("<p>File: ", basename(values$boolean_file_path), "</p>") else "",
        "<h2>Analysis Log</h2>",
